"""
ArguXAIP — An Argumentation-Based Framework for Explaining Scheduled Temporal Plans
...
# ── PAGE CONFIG ──────────────────────────────────────────────
st.set_page_config(
    page_title="ArguXAIP — Bus–Train Plan",
    page_icon="🚌",
    layout="wide",
...
def scenario_fault(scenario_key):
    return {
        "A – Nominal plan": None,
...
def evaluate_cqs(action, scenario_key):
    fault = scenario_fault(scenario_key)
    results = {}
...
# ── Derived structures for instance-specific drill-down ─────────
# Concurrent pairs are derived from the plan data itself (concurrent_with),
# not hardcoded, so this generalises to plans with more than one pair.
CONCURRENT_PAIRS = sorted({
    tuple(sorted((a["id"], c))) for a in PLAN for c in a["concurrent_with"]
})

# Orderings carry the causal label (which effect, which downstream condition)
# alongside the raw predecessor/successor ids, derived once here and reused
# everywhere an ordering needs to be instantiated or displayed.
def _derive_orderings():
    orderings = []
    for succ in PLAN:
        for f in succ["pre_s"]:
            for pred in PLAN:
                ...
                if f in pred["eff_e"]:
                    orderings.append({
                        "pred": pred["id"], "succ": succ["id"],
                        "effect": f, "condition": "start condition of " + succ["action"],
                    })
    return orderings

ORDERINGS = _derive_orderings()

def ordering_label(o):
    pred = next(a for a in PLAN if a["id"] == o["pred"])
    succ = next(a for a in PLAN if a["id"] == o["succ"])
    return f"Actions {o['pred']}→{o['succ']}: {pred['action']} → {succ['action']}"

def pair_label(pair):
    a1 = next(a for a in PLAN if a["id"] == pair[0])
    a2 = next(a for a in PLAN if a["id"] == pair[1])
    return f"Actions {pair[0]} & {pair[1]} — {a1['action']} ∥ {a2['action']} over [{max(a1['start'],a2['start'])},{min(a1['end'],a2['end'])}]"

# Maps each PSA(P) premise to its defeating-side scheme, the critical
# question(s) that attack it, and what kind of target it needs (a time
# point, a single action, a concurrent pair, or an ordering). This is the
# backbone of the premise-first drill-down: pick a premise, get the right
# kind of specific target, then only the critical questions that actually
# apply to that target.
PREMISE_INFO = {
    "P1": {"scheme": "S0", "cqs": [], "target_type": "time",
           "text": "State is fully characterised by S0 at every relevant time point"},
    "P2": {"scheme": "S1", "cqs": ["CQ1", "CQ2"], "target_type": "action",
           "text": "Every action is applicable over its scheduled interval (S1)"},
    "P3": {"scheme": "S2", "cqs": ["CQ3"], "target_type": "action",
           "text": "Every action contributes causally to the goal (S2)"},
    "P4": {"scheme": "S3", "cqs": ["CQ4", "CQ5"], "target_type": "action",
           "text": "Every action is temporally feasible (S3)"},
    "P5": {"scheme": "S4", "cqs": ["CQ6"], "target_type": "pair",
           "text": "Concurrent actions are each individually executable (S4)"},
    "P6": {"scheme": "S5", "cqs": ["CQ7"], "target_type": "pair",
           "text": "Concurrent actions are resource-safe and invariant-compatible (S5)"},
    "P7": {"scheme": "S6", "cqs": ["CQ8"], "target_type": "ordering",
           "text": "All orderings are necessary and justified (S6)"},
    "P8": {"scheme": "S7", "cqs": ["CQ9"], "target_type": "action",
           "text": "All over-all invariants are maintained throughout (S7)"},
}

def cq_applicable(cq_id, action=None):
    """Whether a CQ is applicable (not n/a) for a specific action instance."""
    if cq_id in ("CQ2", "CQ9") and action is not None and not action["inv"]:
        return False
    if cq_id in ("CQ6", "CQ7", "CQ9") and action is not None and not action["concurrent_with"]:
        return False
    return True

def causal_consumers(action):
    """What consumes this action's end effects: a later action's premises, or the goal."""
    consumers = []
    for eff in action["eff_e"]:
        if eff in GOAL:
            consumers.append((eff, "goal", None))
        for other in PLAN:
            if other["id"] == action["id"]:
                continue
            if eff in other["pre_s"]:
                consumers.append((eff, "start condition", other))
            if eff in other["inv"]:
                consumers.append((eff, "invariant", other))
            if eff in other["pre_e"]:
                consumers.append((eff, "end condition", other))
    return consumers

def enabling_actions(action):
    """Actions whose end effects supply this action's start conditions."""
    enablers = []
    for f in action["pre_s"]:
        for other in PLAN:
            if other["id"] == action["id"]:
                continue
            if f in other["eff_e"]:
                enablers.append((f, other))
    return enablers

# ── Scheme instantiation renderers ───────────────────────────────
# Each function renders the premises of ONE scheme, specialised to ONE
# concrete target (a time point, action, concurrent pair, or ordering).
# These are the single source of truth used by the Scheme Inspector, the
# CQ Chatbot, and the PSA(P) drill-down page, so a fix in one place fixes
# all three views.

def render_s0(t_val, scenario_key="A – Nominal plan"):
    fault = scenario_fault(scenario_key)
    state = replayed_state(t_val, fault)
    st.markdown("**Premises:**")
    st.markdown("- ✅ **P1 (Initial state established):** S(0) = I is defined. All fluents have known truth values at t=0.")
...
def render_s1(action_id, scenario_key="A – Nominal plan"):
    a = next(x for x in PLAN if x["id"] == action_id)
    fault = scenario_fault(scenario_key)
    state_s = replayed_state(a["start"], fault)
    state_e = replayed_state(a["end"], fault)
    ...
    if not a["inv"]:
        ...
        icon = "✅" if cq_applicable("CQ2", a) is False or scenario_fault(scenario_key) != "invariant_disruption" else "❌"
        st.markdown(f"{icon} **P2 (Invariant persistence):** The following must hold throughout ({a['start']}, {a['end']}):")
        for f in a["inv"]:
            st.markdown(f"  - `{f}`")
    if not a["pre_e"]:
    ...
    verdict_line = "applicable" if (p1_ok and p3_ok) else "**not** applicable as scheduled"
    st.markdown(f"**Conclusion:** Action {a['id']} ({a['action']}) is {verdict_line} over [{a['start']}, {a['end']}].")

def render_s2(action_id):
    a = next(x for x in PLAN if x["id"] == action_id)
    consumers = causal_consumers(a)
    st.markdown("**Premises:**")
    if not a["eff_e"]:
        st.markdown("❌ **P1 (Action produces an effect):** This action produces no end effects. S2 does not apply.")
        return
    st.markdown(f"✅ **P1 (Action produces an effect):** {a['action']} produces `{', '.join(a['eff_e'])}` at t={a['end']}.")
    if not consumers:
        st.markdown("❌ **P2 (Effect required downstream):** No later action or the goal requires this effect.")
        st.markdown(f"**Conclusion:** Action {a['id']} ({a['action']}) does not causally contribute under S2's premises.")
        return
    st.markdown("✅ **P2 (Effect required downstream):**")
    for eff, kind, other in consumers:
        if kind == "goal":
            st.markdown(f"  - `{eff}` directly satisfies the goal `{GOAL[0]}`.")
        else:
            st.markdown(f"  - `{eff}` is required as the {kind} of Action {other['id']} ({other['action']}).")
    st.markdown(f"✅ **P3 (Effect genuinely consumed):** S1 confirms Action {a['id']} executes and its end effects fire at t={a['end']}, so the dependents above genuinely receive `{', '.join(a['eff_e'])}`.")
    st.markdown(f"**Conclusion:** Action {a['id']} ({a['action']}) causally contributes to the plan.")

def render_s3(action_id, scenario_key="A – Nominal plan"):
    a = next(x for x in PLAN if x["id"] == action_id)
    enablers = enabling_actions(a)
    fault = scenario_fault(scenario_key)
    st.markdown("**Premises:**")
    if not enablers:
        st.markdown("✅ **P2 (Enabling actions finish in time):** No other action supplies this action's start conditions — they hold from the initial state.")
    else:
        st.markdown("**P2 (Enabling actions finish in time):**")
        for f, other in enablers:
            eff_end = other["end"]
            if fault == "bus_delayed" and other["id"] == 2:
                eff_end = 8
            ok = eff_end <= a["start"]
            icon = "✅" if ok else "❌"
            st.markdown(f"  - {icon} Action {other['id']} ({other['action']}) produces `{f}` at e={eff_end}, needed by Action {a['id']} at s={a['start']}.")
    st.markdown(f"✅ **P3 (Own window respects timing constraints):** Action {a['id']}'s scheduled interval [{a['start']},{a['end']}] respects the makespan deadline ({DEADLINE}) and any release-time constraint.")
    ok_overall = all((other["end"] if not (fault == "bus_delayed" and other["id"] == 2) else 8) <= a["start"] for f, other in enablers) if enablers else True
    verdict_line = "temporally feasible" if ok_overall else "**not** temporally feasible as scheduled"
    st.markdown(f"**Conclusion:** Action {a['id']} ({a['action']}) is {verdict_line}.")

def render_s4(pair):
    a1 = next(x for x in PLAN if x["id"] == pair[0])
    a2 = next(x for x in PLAN if x["id"] == pair[1])
    overlap_start = max(a1["start"], a2["start"])
    st.markdown(f"**Target:** Actions {a1['id']} & {a2['id']} — `{a1['action']}` ∥ `{a2['action']}` over [{overlap_start},{min(a1['end'],a2['end'])}]")
    st.markdown("**Premises:**")
    st.markdown(f"✅ **P1 (Actions identified as concurrent):** Actions {a1['id']} and {a2['id']} share an overlapping execution interval.")
    st.markdown("✅ **P2 (Each action individually applicable):**")
    st.markdown(f"  - S1 confirms Action {a1['id']} is individually applicable at s={a1['start']}: replayed state S({a1['start']}) contains its start conditions.")
    st.markdown(f"  - S1 confirms Action {a2['id']} is individually applicable at s={a2['start']}: replayed state S({a2['start']}) contains its start conditions.")
    st.markdown(f"**Conclusion:** Actions {a1['id']} and {a2['id']} can each legally begin at t={overlap_start}. Whether running them together is resource-safe and invariant-compatible is established by S5.")

def render_s5(pair, scenario_key="A – Nominal plan"):
    a1 = next(x for x in PLAN if x["id"] == pair[0])
    a2 = next(x for x in PLAN if x["id"] == pair[1])
    st.markdown(f"**Target:** Actions {a1['id']} & {a2['id']} — `{a1['action']}` ∥ `{a2['action']}` over [{max(a1['start'],a2['start'])},{min(a1['end'],a2['end'])}]")
    st.markdown("**Premises:**")
    st.markdown(f"✅ **P1 (Both actions concurrently executable):** S4 confirms both are individually applicable and their intervals overlap.")
    resource_ok = scenario_fault(scenario_key) != "resource_conflict"
    icon = "✅" if resource_ok else "❌"
    if a1["resource"] == "None" or a2["resource"] == "None" or a1["resource"] != a2["resource"]:
        st.markdown(f"{icon} **P2 (No resource conflict):** Action {a1['id']} holds `{a1['resource']}`. Action {a2['id']} holds `{a2['resource']}`. These resource sets are disjoint" + (" — no conflict." if resource_ok else ", but a second agent claims the same resource concurrently in this scenario."))
    else:
        st.markdown(f"{icon} **P2 (No resource conflict):** Both actions require `{a1['resource']}` — a resource conflict.")
    st.markdown(f"✅ **P3 (Mutual invariant compatibility):** Neither action's effects delete a fluent the other requires as an invariant.")
    st.markdown(f"✅ **P4 (Concurrency justified):** Running Actions {a1['id']} and {a2['id']} concurrently reduces the plan's makespan compared with running them sequentially.")
    verdict_line = "resource-feasible, invariant-safe, and justified" if resource_ok else "**not** resource-feasible in this scenario"
    st.markdown(f"**Conclusion:** The concurrent scheduling of Actions {a1['id']} and {a2['id']} is {verdict_line}.")
...
def render_s6(o):
    ...
    st.markdown(f"✅ **P2 (Ordering is causally motivated):** Action {pred['id']} ({pred['action']}) produces `{o['effect']}`, which is a {o['condition']}.")
    st.markdown(f"✅ **P3 (Reversal harms the plan):** If Action {succ['id']} started before Action {pred['id']} ended, `{o['effect']}` would be absent from the replayed state at s_{succ['id']}={succ['start']}. Action {succ['id']} would be inapplicable and the causal chain to the goal would be broken.")
    st.markdown(f"**Conclusion:** The ordering of Action {pred['id']} before Action {succ['id']} is necessary and justified.")

def render_s7(action_id, scenario_key="A – Nominal plan"):
    a = next(x for x in PLAN if x["id"] == action_id)
    if not a["inv"]:
        st.info(f"Action {a['id']} ({a['action']}) has no over-all invariants. S7 does not apply.")
        return
    disrupted = scenario_fault(scenario_key) == "invariant_disruption"
    st.markdown("**Premises:**")
    st.markdown(f"✅ **P1 (Invariant exists):** inv({a['action']}) = {{{', '.join(a['inv'])}}}")
    icon = "❌" if disrupted else "✅"
    st.markdown(f"{icon} **P2 (Invariant holds and is not disrupted):** `{a['inv'][0]}` must hold in the replayed state at s={a['start']} and remain undisturbed throughout ({a['start']}, {a['end']})." + (" In this scenario a concurrent action removes it mid-interval." if disrupted else ""))
    ...
    verdict_line = "necessary and justified" if not disrupted else "**violated** in this scenario"
    st.markdown(f"**Conclusion:** The continuous maintenance of `{a['inv'][0]}` throughout ({a['start']}, {a['end']}) is {verdict_line}.")

def resolve_defeat_target(cq_id, target, action_obj):
    """The defeating scheme is sometimes a different scheme, over a different
    kind of target, than the one being attacked (e.g. CQ4/CQ5 attack S3 over
    an action, but are defeated by S6 over an ordering). This resolves the
    right concrete target for the defeating scheme, derived from the data
    rather than hardcoded, so it generalises beyond this one bus-train plan."""
    if cq_id in ("CQ1", "CQ2", "CQ3"):
        return target
    if cq_id in ("CQ4", "CQ5"):
        matches = [o for o in ORDERINGS if action_obj and o["succ"] == action_obj["id"]]
        return matches[0] if matches else (ORDERINGS[0] if ORDERINGS else None)
    if cq_id in ("CQ6", "CQ7"):
        return target
    if cq_id == "CQ8":
        return target["pred"] if isinstance(target, dict) else target
    if cq_id == "CQ9":
        matches = [p for p in CONCURRENT_PAIRS if action_obj and action_obj["id"] in p]
        return matches[0] if matches else (CONCURRENT_PAIRS[0] if CONCURRENT_PAIRS else None)
    return target

def defeat_target_label(cq_id, defeat_target):
    if cq_id in ("CQ6", "CQ7", "CQ9"):
        return pair_label(defeat_target) if defeat_target else "—"
    if isinstance(defeat_target, dict):
        return ordering_label(defeat_target)
    return f"Action {defeat_target}" if defeat_target is not None else "—"

def render_scheme_instance(scheme, target, scenario_key="A – Nominal plan"):
    """Dispatch to the right renderer for a scheme + target combination."""
    if scheme == "S0":
        render_s0(target, scenario_key)
    elif scheme == "S1":
        render_s1(target, scenario_key)
    elif scheme == "S2":
        render_s2(target)
    elif scheme == "S3":
        render_s3(target, scenario_key)
    elif scheme == "S4":
        render_s4(target)
    elif scheme == "S5":
        render_s5(target, scenario_key)
    elif scheme == "S6":
        render_s6(target)
    elif scheme == "S7":
        render_s7(target, scenario_key)
...
- **{cq_id}** attacks **{cq['attacks']} ({SCHEMES[cq['attacks']]})**,
  ...
  **S8 — Plan Summary Argument**, *{cq['also_attacks_s8']}*.
  ...
  invalid.
    ...
    | **Explain PSA(P)** | Premise-first drill-down: pick a PSA(P) premise → pick the specific state, action, pair, or ordering it concerns → pick the CQ that applies → see the scheme instantiated for exactly that selection |
    | **CQ Chatbot** | Scheme-first navigation: pick a scheme → pick a CQ that attacks it → pick the specific action, pair, or ordering, and see the full challenge–response dialogue with the specialised scheme instantiation |
...
# ══════════════════════════════════════════════════════════════
    ...
    st.caption("To challenge any of these premises for a specific state, action, pair, or ordering, use **🧩 Explain PSA(P)** in the sidebar.")
...
# ══════════════════════════════════════════════════════════════
#  PAGE 1.5 — EXPLAIN PSA(P) (premise-first drill-down)
# ══════════════════════════════════════════════════════════════

elif page == "🧩 Explain PSA(P)":
    st.title("🧩 Explain PSA(P) — Premise-First Drill-Down")
    st.markdown("""
    Start from the **Plan Summary Argument** itself. Pick one of its eight premises,
    then pick the **specific** state, action, pair, or ordering that premise is about,
    then pick the **critical question** that challenges it. The system generates a
    scheme instance specialised to exactly what you picked, not a generic explanation.
    """)
    ...
    st.markdown("---")
    st.markdown("### Step 1 — Pick a premise of PSA(P)")
    premise_options = [f"{pid} — {info['text']}" for pid, info in PREMISE_INFO.items()]
    sel_premise_str = st.selectbox("Premise:", premise_options)
    sel_premise = sel_premise_str.split(" — ")[0]
    info = PREMISE_INFO[sel_premise]
    scheme = info["scheme"]

    st.markdown(f'<div class="scheme-box"><b>{scheme} — {SCHEMES[scheme]}</b><br><small>{SCHEME_DESC[scheme]}</small></div>', unsafe_allow_html=True)

    st.markdown("### Step 2 — Pick the specific target this premise is about")
    target = None
    action_obj = None
    if info["target_type"] == "time":
        t_opts = [0, 2, 7, 10, 11, 19]
        target = st.selectbox("Replayed state time point:", t_opts, format_func=lambda t: f"S(t={t})")
    elif info["target_type"] == "action":
        if sel_premise == "P8":
            candidates = [a for a in PLAN if a["inv"]]
            if not candidates:
                st.info("No action in this plan has an over-all invariant, so P8 / S7 does not apply to any specific action.")
                candidates = None
        else:
            candidates = PLAN
        if candidates:
            action_obj = st.selectbox("Action:", candidates, format_func=lambda a: f"Action {a['id']} – {a['action']}")
            target = action_obj["id"]
    elif info["target_type"] == "pair":
        if not CONCURRENT_PAIRS:
            st.info("This plan has no concurrent actions, so this premise does not apply to any specific pair.")
        else:
            pair_sel = st.selectbox("Concurrent pair:", CONCURRENT_PAIRS, format_func=pair_label)
            target = pair_sel
            action_obj = next(a for a in PLAN if a["id"] == pair_sel[0])
    elif info["target_type"] == "ordering":
        ord_sel = st.selectbox("Ordering (predecessor → successor):", ORDERINGS, format_func=ordering_label)
        target = ord_sel
        action_obj = next(a for a in PLAN if a["id"] == ord_sel["pred"])

    if target is not None:
        st.markdown("### Step 3 — Pick the critical question to raise")
        applicable_cqs = [cq_id for cq_id in info["cqs"] if cq_applicable(cq_id, action_obj)]
        if not applicable_cqs:
            st.markdown(f'<div class="cq-na">ℹ️ No critical question applies to this specific selection — its premises hold by construction (e.g. S0 is the evidential foundation with no defeating role, or the selected instance is outside the scope of the relevant CQs).</div>', unsafe_allow_html=True)
        else:
            cq_options = [f"{cq_id}: {CQS[cq_id]['challenge'][:80]}…" for cq_id in applicable_cqs]
            sel_cq_str = st.selectbox("Critical Question:", cq_options)
            sel_cq_id = sel_cq_str.split(":")[0].strip()
            sel_cq = CQS[sel_cq_id]

            outcome = evaluate_cqs(action_obj, scenario_key).get(sel_cq_id, "n/a") if action_obj else "n/a"

            st.markdown("---")
            st.markdown("### Argumentation Response")
            st.markdown("**🔴 Challenge (Layer 3 — R1a + R1b):**")
            st.markdown(f"""
- **{sel_cq_id}** attacks **{sel_cq['attacks']} ({SCHEMES[sel_cq['attacks']]})** via R1a, targeting *{sel_cq['attacks_premise']}*
- Simultaneously attacks **S8 (Plan Summary Argument)** via R1b, undermining *{sel_cq['also_attacks_s8']}*
- **Challenge:** {sel_cq['challenge']}
""")

            if outcome == "n/a":
                st.markdown(f'<div class="cq-na">ℹ️ <b>{sel_cq_id}: Not Applicable</b><br>{sel_cq["na_when"]}</div>', unsafe_allow_html=True)
            else:
                defeating = sel_cq["defeated_by"]
                defeat_target = resolve_defeat_target(sel_cq_id, target, action_obj)
                st.markdown("**🟢 Defeating Justification (Layer 4 — R2), specialised to your selection:**")
                st.markdown(f"*Premise doing the work: {sel_cq['defeated_by_premise']}*")
                st.markdown(sel_cq["how"])
                st.markdown(f"**{defeating} instantiated for {defeat_target_label(sel_cq_id, defeat_target)}:**")
                render_scheme_instance(defeating, defeat_target, scenario_key)

                if outcome == "defeated":
                    st.markdown(f'<div class="cq-defeated">✅ <b>{sel_cq_id} DEFEATED</b> for this specific selection — {defeating} provides a complete answer. The attack on PSA(P) is blocked.</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="cq-succeeds">❌ <b>{sel_cq_id} SUCCEEDS</b> for this specific selection — {defeating} cannot answer here. The attack on PSA(P) is unblocked. PSA(P) is REJECTED.</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    if not relevant_cqs:
        ...
        # Step 3: Choose specific action / pair / ordering — the actual
        # selection now drives everything downstream (previously S4/S5/S6
        # always defaulted to action 3 regardless of what was picked here).
        st.markdown("### Step 3 — Select the specific action, pair, or ordering to inspect")

        if selected_scheme in ("S4", "S5"):
            pair_sel = st.selectbox("Concurrent pair:", CONCURRENT_PAIRS, format_func=pair_label)
            attacked_target = pair_sel
            action = next(a for a in PLAN if a["id"] == pair_sel[0])
        elif selected_scheme == "S6":
            ord_sel = st.selectbox("Ordering (predecessor → successor):", ORDERINGS, format_func=ordering_label)
            attacked_target = ord_sel
            action = next(a for a in PLAN if a["id"] == ord_sel["succ"])
        else:
            action_target = st.selectbox("Action:", PLAN, format_func=lambda a: f"Action {a['id']} – {a['action']}")
            attacked_target = action_target["id"]
...
""")
        ...
        if outcome == "n/a":
            ...
            # Defeating response — specialised to the exact target selected above
            defeating = selected_cq['defeated_by']
            defeat_target = resolve_defeat_target(selected_cq_id, attacked_target, action)
...
{selected_cq['how']}
""")
            st.markdown(f"**{defeating} instantiated for {defeat_target_label(selected_cq_id, defeat_target)}:**")
            render_scheme_instance(defeating, defeat_target, scenario_key)
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown("Inspect any scheme (S0–S8) instantiated for a specific action, replayed state time point, concurrent pair, or ordering. Select the scheme and its specific target below. Instantiations reflect the scenario currently selected in the sidebar.")
    ...
    # Dynamic target selector based on scheme — widget objects are used
    # directly rather than parsed back out of display strings.
    sel_target = None
    with col2:
        if sel_scheme == "S0":
            sel_target = st.selectbox("Replayed state time point:", [0, 2, 7, 10, 11, 19], format_func=lambda t: f"S(t={t})")
        elif sel_scheme in ("S1", "S2", "S3", "S7"):
            action_target = st.selectbox("Action:", PLAN, format_func=lambda a: f"Action {a['id']} – {a['action']}")
            sel_target = action_target["id"]
        elif sel_scheme in ("S4", "S5"):
            sel_target = st.selectbox("Concurrent pair:", CONCURRENT_PAIRS, format_func=pair_label)
        elif sel_scheme == "S6":
            sel_target = st.selectbox("Ordering (predecessor → successor):", ORDERINGS, format_func=ordering_label)
        elif sel_scheme == "S8":
            st.text_input("Target:", value="Full plan — all 6 actions", disabled=True)
        else:
            sel_target = "N/A"
    ...
    target_label = (
        ordering_label(sel_target) if sel_scheme == "S6" else
        pair_label(sel_target) if sel_scheme in ("S4", "S5") else
        f"S(t={sel_target})" if sel_scheme == "S0" else
        f"Action {sel_target}" if sel_scheme in ("S1", "S2", "S3", "S7") else
        "Full plan — all 6 actions"
    )
    st.markdown(f"**Instantiated for:** `{target_label}` — scenario `{scenario_key}`")

    if sel_scheme == "S8":
        st.markdown("**This is the Plan Summary Argument PSA(P).**")
        st.markdown("PSA(P) integrates S0–S7 across all six actions. It is accepted under grounded semantics if and only if every CQ is defeated.")
        verdict_icon = "✅" if verdict == "ACCEPTED" else "❌"
        st.markdown(f"**Current verdict for scenario '{scenario_key}':** {verdict_icon} **{verdict}**")
    elif sel_target is None or (isinstance(sel_target, str) and sel_target == "N/A"):
        st.info("No specific target applies to this scheme.")
    else:
        render_scheme_instance(sel_scheme, sel_target, scenario_key)
